...
export default function OrderList({ searchQuery = '' }: { searchQuery?: string }) {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">상품 주문 현황</h1>
          <p className="text-gray-500">고객들이 제출한 상품 주문서 리스트입니다.</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredResponses.length > 0 ? (
                filteredResponses.map((res) => {
                  const data = res.data || {};
                  const customerName = data['고객명'] || '-';
                  const customerPhone = data['연락처'] || '-';
                  const productName = data['상품명'] || '-';
                  const depositorName = data['입금자명'] || '-';
                  ...
                  return (
                    <tr key={res.id} className="hover:bg-gray-50/50 transition">
                      ...
                      <td className="px-5 py-4">
                        <div className="text-sm font-bold text-slate-800 mb-1">{customerName}</div>
                        <div className="text-sm text-slate-500 font-mono">{customerPhone}</div>
                      </td>
                      <td className="px-5 py-4">
                        <div className="text-sm font-bold text-[#5C4D3C] mb-1">{productName}</div>
                        <div className="text-sm text-gray-600">{depositorName}</div>
                      </td>
                      <td className="px-5 py-4">
                        <button 
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    ...
                    <h3 className="text-gray-900 font-medium text-lg">아직 접수된 주문이 없습니다</h3>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] flex flex-col shadow-2xl">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-gray-50 rounded-t-2xl">
              <h2 className="text-lg font-bold text-gray-900">상품 주문 상세 정보</h2>
