...
function updateReservationModal() {
  ...
  const splitPart = content.split('{selectedResponse && (');
  if (splitPart.length < 2) return;
  const before = splitPart[0];
...
`;
  ...
  let finalContent = before + newModal;
  if (!finalContent.includes('import { CheckCircle')) {
     finalContent = finalContent.replace("import { Calendar, Trash2, Printer, RotateCcw, X, Search, ChevronLeft, ChevronRight } from 'lucide-react';", "import { Calendar, Trash2, Printer, RotateCcw, X, Search, ChevronLeft, ChevronRight, CheckCircle } from 'lucide-react';");
  }
...
`;
  ...
  let finalContent = before + newModal;
  if (!finalContent.includes('import { CheckCircle')) {
     finalContent = finalContent.replace("import { ShoppingCart, Trash2, Printer, RotateCcw, X, Search, ChevronLeft, ChevronRight } from 'lucide-react';", "import { ShoppingCart, Trash2, Printer, RotateCcw, X, Search, ChevronLeft, ChevronRight, CheckCircle } from 'lucide-react';");
  }
  fs.writeFileSync(file, finalContent);
