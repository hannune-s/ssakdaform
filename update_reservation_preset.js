...
const targetStr = `    } else if (id === 'reservation-preset') {
      setData({
...
const newStr = `    } else if (id === 'reservation-preset') {
      setData({
        storeName: '내 매장 이름 (기본 설정)',
        formTitle: '간편한 매장 예약 신청',
        ...
        fields: [
          ...
          { id: 7, type: 'text', label: '입금자명 (예약금 입금 시)', placeholder: '주문자와 동일하면 비워두세요', required: false },
...
if (c.includes(targetStr)) {
  ...
  console.log('Successfully updated reservation preset');
} else {
